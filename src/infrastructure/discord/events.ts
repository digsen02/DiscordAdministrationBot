...
export function installDiscordEvents(client: Client, db: AppDatabase, handler: InteractionHandler, queue: RefreshQueue, logger: Logger): void {
  ...
  client.on(Events.InteractionCreate, (interaction) => { if (interaction.isAutocomplete()) void handler.handleAutocomplete(interaction); else if (interaction.isChatInputCommand()) void handler.handle(interaction); else if (interaction.isModalSubmit()) void handler.handleModal(interaction); else if (interaction.isButton()) void handler.handleButton(interaction); else if (interaction.isStringSelectMenu()) void handler.handleSelect(interaction); else if (interaction.isRoleSelectMenu()) void handler.handleRoleSelect(interaction); else if (interaction.isChannelSelectMenu()) void handler.handleChannelSelect(interaction); });
