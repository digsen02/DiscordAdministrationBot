...
export function templatePanel(template: Template, organizationName: string, owner: string, usageCount = 0, notice?: string, browserPage = 0) {
  const target = pageTarget(template.id, browserPage);
  const b = (label: string, action: string, style = ButtonStyle.Secondary) => new ButtonBuilder().setCustomId(managementId('tpl', action, target, owner)).setLabel(label).setStyle(style);
  const actions = new StringSelectMenuBuilder().setCustomId(managementId('tpl', 'actions', target, owner)).setPlaceholder('기타 작업').addOptions(
  ...
  return { ...(notice ? { content: notice } : {}), embeds: [new EmbedBuilder().setTitle(`${organizationName} · 템플릿 · ${template.name}`).addFields(
    ...
    new ActionRowBuilder<ButtonBuilder>().addComponents(b('미리보기', 'preview', ButtonStyle.Primary), b('내용 수정', 'edit'), b('이 템플릿으로 게시물 만들기', 'createpub')),
